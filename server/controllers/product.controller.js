...
export async function uploadImages(request, response) {
  try {
    let imagesArr = []; // ✅ Make it local to each request
    ...
    return response.status(200).json({
      images: imagesArr // ✅ Return the local array
    });
...
export async function createproduct(request, response) {
  try {
    // ✅ Get images from request body (uploaded via separate endpoint)
...
// Alternative: Create product with direct image upload
export async function createProductWithImages(request, response) {
  try {
    let imagesArr = [];
    ...
    if (request.files && request.files.length > 0) {
      ...
      for (let i = 0; i < request.files.length; i++) {
        const uploadResult = await cloudinary.uploader.upload(request.files[i].path, options);
        imagesArr.push(uploadResult.secure_url);
    ...
    const product = new ProductModel({
      name: request.body.name,
      description: request.body.description,
      images: imagesArr, // Use uploaded images
      brand: request.body.brand,
      price: request.body.price,
      oldePrice: request.body.oldePrice,
      catName: request.body.catName,
...
export async function getAllProducts(request, response) {
  try {
    ...
    let perPage = parseInt(request.query.perPage, 10);

    // fallback if perPage is missing or invalid
    if (Number.isNaN(perPage) || perPage <= 0) {
      perPage = 10;
    }

    const totalPosts = await ProductModel.countDocuments();
    const totalPages = perPage ? Math.ceil(totalPosts / perPage) : 0;

    // if there are products and requested page is out of range
...
export async function deleteMultipleProducts(request, response) {
  try {
        ...
        for (const imgUrl of product.images) {
          ...
          if (publicId) {
            try {
              ...
              console.error(
                `Failed to delete image ${imgUrl} for product ${product._id}:`,
                cloudinaryError
              );
            }
          }
    ...
    return response.status(200).json({
      message: `${idsToDelete.length} product(s) and related images deleted successfully`,
      error: false,
      success: true,
...
export async function updateProduct(request, response) {
  try {
    ...
    if (request.files && request.files.length > 0) {
      ...
      // If images array is provided in body (from admin edit form), use it
      const bodyImages =
